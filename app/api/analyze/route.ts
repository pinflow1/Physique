...
export async function POST(request: NextRequest) {
  try {
    ...
    // 2. Parse form data — read ALL buffers once up front
    ...
    if (goalImageFile && !allowedTypes.includes(goalImageFile.type)) {
      return NextResponse.json({ error: 'Invalid goal image type.' }, { status: 400 })
    }
    ...
    // Read buffers NOW before any branching
    ...
    // 10. Deduct credits after successful AI — conditional to prevent race conditions
    const { error: deductErr, data: deductedData } = await supabase
      .from('users')
    ...
    if (deductErr || !deductedData || deductedData.length === 0) {
    ...
    if (resultErr) {
      console.error('Result save error:', resultErr)
    }
